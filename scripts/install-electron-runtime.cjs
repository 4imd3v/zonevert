...
function isInstalled() {
    try {
        ...
        return (
            distVersion === electronPackage.version && pathFile === platformPath
        );
    } catch {
        return false;
    }
...
async function main() {
    ...
    const { downloadArtifact } = await import(
        pathToFileURL(getPackagePath).href
    );
    const platform =
        process.env.ELECTRON_INSTALL_PLATFORM ||
        process.env.npm_config_platform ||
...
function extractArchive(zipPath, distPath) {
    if (!zipPath) {
        throw new Error("Electron download did not return an archive path.");
    }
